...
function Inventory(props) {
  ...
  const [coin, setCoin] = createSignal("red");
  const [baseRate, setBaseRate] = createSignal(0.1);
  ...
  function toggleItemSelection(item) {
    setSelectedItems((prevSelectedItems) => {
      ...
      if (!isItemAlreadySelected) {
        const isSameGame = prevSelectedItems.every(
          (selectedItem) => selectedItem.game === item.game
        );

        if (!isSameGame) {
          createNotification(
            "error",
            "You can't select items for different games!"
          );
  ...
  function calculateTotalValue(items) {
    const itemValue = items().reduce((total, item) => total + item.value, 0);

    const isMM2 = items().every((item) => item.game.toLowerCase() === "mm2");
    const currentRate = isMM2 ? baseRate() / 1000 : baseRate() / 1000;

    const valueInDollars = itemValue * currentRate;
  ...
  function getTotalAmount() {
    ...
    const isAllMM2 = items.every((item) => item.game.toLowerCase() === "mm2");
    const isAllDahood = items.every(
      (item) => item.game.toLowerCase() === "dahood"
    );

    let currentBaseRate;
    if (isAllMM2) {
      currentBaseRate = 3.5 / 1000;
    } else if (isAllDahood) {
      currentBaseRate = 0.5 / 1000;
    } else {
      currentBaseRate = baseRate() / 100000;
    }

    const amountInDollars = totalValue * currentBaseRate;
    const standardConversionRate = 3 / 100000;
    const equivalentAmount = amountInDollars / standardConversionRate;
  ...
  const isLoading = () =>
    items.loading || items() === null || !items() || !items;
      ...
      <div class="modal fadein" onClick={() => props.close()}>
        <div class="coinflip-create" onClick={(e) => e.stopPropagation()}>
          <div class="header">
            ...
            <p class="title">
              <img src="/assets/icons/inventoryicon.svg" height="25" alt="" />
              Sell Items
            </p>
          </div>
          ...
          <div class="footer">
            <div class="selected info">
              <p style="white-space: nowrap;">
                SELECTED {selectedCount(selectedItems())}/50
              </p>
            </div>
            ...
            <div className="footer__block">
              <div>
                <input
                  ...
                  min={
                    selectedItems().every(
                      (item) => item.game.toLowerCase() === "mm2"
                    )
                      ? "0.01"
                      : "0.01"
                  }
              ...
              <div className="cost">
                ...
                <p>
                  {Math.floor(
                    calculateTotalValue(selectedItems)
                  )?.toLocaleString(undefined, {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                  })}
                  <span className="gray">
                    .{getCents(calculateTotalValue(selectedItems))}
                  </span>
                </p>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (res.success) {
                    createNotification(
                      "success",
                      "Successfully listed your items on the marketplace!",
                      ""
                    );
                    props?.close();
                  }
